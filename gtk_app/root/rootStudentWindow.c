...
void rootStudentWindowInit(){
    ...
    gtk_widget_set_usize(rootStudentWindow,400,400);
    ...
    for(i = 0;i < len;i++){
        ...
        char content[100];
        sprintf(content,"%d\t%s\t%s\t%s",i+1,stu[i].sno,stu[i].sname,stu[i].sex);
        label = gtk_label_new(content);
        gtk_box_pack_start((GtkBox *) hbox, label, FALSE, FALSE, 5);

        button = gtk_button_new_with_label("delete");
        gtk_box_pack_start((GtkBox *) hbox, button, FALSE, FALSE, 5);
        strcpy(sno[i],stu[i].sno);
        g_signal_connect(GTK_OBJECT(button),"pressed",GTK_SIGNAL_FUNC(rootStudentDelete),sno[i]);
